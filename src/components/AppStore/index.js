...
class AppStore extends Component {
  ...
  getActiveTabs = searchResults => {
    const {activeTab} = this.state
    console.log(activeTab)
    const filteredRes = searchResults.filter(
      eachTab => eachTab.category === activeTab,
    )
    return filteredRes
  }
  ...
  searchResults = () => {
    ...
    const results = appsList.filter(eachApp =>
  ...
  render() {
    ...
    const filteredRes = this.getActiveTabs(searchResults)
      ...
      <div className="bg-container">
        ...
        <ul className="all-apps">
          {filteredRes.map(eachApp => (
            <AppItem appData={eachApp} key={eachApp.appId} />
          ))}
        </ul>
      </div>
    )
